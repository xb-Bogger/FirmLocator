...
void asr_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "asr_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    ...
    if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        ...
        uint32_t shift = CAST2_USE(src[1]->node)->val.dword & 0x1F;
        int32_t val = (int32_t)CAST2_USE(src[0]->node)->val.dword;
        vt.dword = (uint32_t)(val >> shift);
     ...
     if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        !CAST2_USE(src[1]->node)->val_known &&
        CAST2_USE(src[0]->node)->val_known) {
        int32_t shift = CAST2_USE(src[0]->node)->val.dword;
        int32_t val = (int32_t)CAST2_DEF(dst[0]->node)->afterval.dword;
        uint32_t tmp = 0;
        while(val != -1 && val !=0 && val != shift){
            tmp++;
            val = val >> 1;
        }
        vt.dword = tmp;
...
void lsl_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "lsl_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 2 && ndef == 1);
...
void lsr_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "lsr_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 2 && ndef == 1);
    ...
    if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        ...
        CAST2_USE(src[1]->node)->val_known) {
        uint32_t shift = CAST2_USE(src[1]->node)->val.dword & 0x1F;
        uint32_t val = CAST2_USE(src[0]->node)->val.dword;
        vt.dword = val >> shift;
        assert_val(dst[0], vt, false);
    }
     ...
     if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        ...
        CAST2_USE(src[0]->node)->val_known) {
        int32_t shift = CAST2_USE(src[0]->node)->val.dword;
        uint32_t val = (int32_t)CAST2_DEF(dst[0]->node)->afterval.dword;
        uint32_t tmp = 0;
        while(val != -1 && val !=0 && val != shift){
            tmp++;
            val = val >> 1;
    ...
    if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        ...
        CAST2_USE(src[1]->node)->val_known) {
        uint32_t shift = CAST2_USE(src[1]->node)->val.dword & 0x1F;
        uint32_t val = CAST2_DEF(dst[0]->node)->afterval.dword;
        vt.dword = val << shift;
        assign_use_value(src[0], vt);
        add_to_uselist(src[0], re_uselist);
    }
...
void ror_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "ror_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 2 && ndef == 1);
    ...
    if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        ...
        uint32_t val = CAST2_USE(src[0]->node)->val.dword;
        uint32_t shift = CAST2_USE(src[1]->node)->val.dword & 0x1F;
        vt.dword = (val >> shift) | (val << (32 - shift));
        assert_val(dst[0], vt, false);
    }
...
void rrx_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "rrx_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 2 && ndef == 1);
    ...
    if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        ...
        CAST2_USE(src[1]->node)->val_known) {
        uint32_t val = CAST2_USE(src[0]->node)->val.dword;
        uint32_t carry = CAST2_USE(src[1]->node)->val.dword & 0x1;
        vt.dword = (val >> 1) | (carry << 31);
        assert_val(dst[0], vt, false);
    }
...
void adr_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "adr_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 1 && ndef == 1);

    // ADR Rd, label  ==> Rd = PC + imm
    // 1 verify
    if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        CAST2_USE(src[0]->node)->val_known) {
        vt.dword = CAST2_USE(src[0]->node)->val.dword;
        assert_val(dst[0], vt, false);
    }
    ...
    if ((CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
        !CAST2_USE(src[0]->node)->val_known) {
        vt.dword = CAST2_DEF(dst[0]->node)->afterval.dword;
        assign_use_value(src[0], vt);
        add_to_uselist(src[0], re_uselist);
    }
}
...
void bic_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "bic_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 2 && ndef == 1);
...
re_list_t *entry;
    ...
    assert(nuse == 2 && ndef == 1);// CBY for eor r1, r1; nuse = 1?
...
void mla_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "mla_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    ...
    if (CAST2_USE(src[0]->node)->val_known &&
        ...
        vt.dword = CAST2_USE(src[0]->node)->val.dword * CAST2_USE(src[1]->node)->val.dword +
                   CAST2_USE(src[2]->node)->val.dword;
        if (CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) {
            assert_val(dst[0], vt, false);
        } else {
            assign_def_after_value(dst[0], vt);
...
void mls_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    traverse_inst_operand(inst, src, dst, re_uselist, re_deflist, &nuse, &ndef);
    LOG(stdout, "mls_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 3 && ndef == 1);

    if (CAST2_USE(src[0]->node)->val_known &&
        CAST2_USE(src[1]->node)->val_known &&
        CAST2_USE(src[2]->node)->val_known) {
        vt.dword = CAST2_USE(src[2]->node)->val.dword -
                   (CAST2_USE(src[0]->node)->val.dword * CAST2_USE(src[1]->node)->val.dword);
        if(CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) assert_val(dst[0], vt, false);
        else {
            assign_def_after_value(dst[0], vt);
            add_to_deflist(dst[0], re_deflist);
        }
...
void movw_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    traverse_inst_operand(inst, src, dst, re_uselist, re_deflist, &nuse, &ndef);
    LOG(stdout, "movw_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 1 && ndef == 1);
    if (CAST2_DEF(dst[0]->node)->val_stat & AfterKnown && CAST2_USE(src[0]->node)->val_known) {
        vt.dword = CAST2_USE(src[0]->node)->val.dword & 0xFFFF;
        assign_def_after_value(dst[0], vt);
        add_to_deflist(dst[0], re_deflist);
    }
    if (!(CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) && CAST2_USE(src[0]->node)->val_known) {
...
void mul_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "mul_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 2 && ndef == 1);
...
void strb_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    // CBY hard to recover source from destination?
    // if (!CAST2_USE(src[0]->node)->val_known &&
    //      (CAST2_DEF(dst[0]->node)->val_stat & AfterKnown)){
    //         vt = CAST2_DEF(dst[0]->node)->afterval;
    //         assign_use_value(src[0],vt);
...
void ubfx_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    LOG(stdout, "ubfx_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 3 && ndef == 1);
    ...
    if (CAST2_DEF(dst[0]->node)->val_stat & AfterKnown &&
        ...
        CAST2_USE(src[2]->node)->val_known) {
            uint32_t val = CAST2_USE(src[0]->node)->val.dword;
            uint32_t lsb = CAST2_USE(src[1]->node)->val.dword;
            uint32_t width = CAST2_USE(src[2]->node)->val.dword;
            vt.dword = (val >> lsb) & ((1U << width) - 1);
            assert_val(dst[0], vt, false);
    }
...
void udiv_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    traverse_inst_operand(inst, src, dst, re_uselist, re_deflist, &nuse, &ndef);
    LOG(stdout, "udiv_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 2 && ndef == 1);
    if (CAST2_DEF(dst[0]->node)->val_stat & AfterKnown &&
        CAST2_USE(src[0]->node)->val_known &&
        CAST2_USE(src[1]->node)->val_known &&
        CAST2_USE(src[1]->node)->val.dword != 0) {
        vt.dword = CAST2_USE(src[0]->node)->val.dword / CAST2_USE(src[1]->node)->val.dword;
        assert_val(dst[0], vt, false);
    }
    if (!(CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) &&
    ...
    if (CAST2_DEF(dst[0]->node)->val_stat & AfterKnown &&
        CAST2_USE(src[0]->node)->val_known &&
        !CAST2_USE(src[1]->node)->val_known &&
        CAST2_DEF(dst[0]->node)->afterval.dword != 0) {
        vt.dword = CAST2_USE(src[0]->node)->val.dword / CAST2_DEF(dst[0]->node)->afterval.dword;
        assign_use_value(src[1], vt);
        add_to_uselist(src[1], re_uselist);
    }
...
void umull_resolver(re_list_t* inst, re_list_t* re_deflist, re_list_t* re_uselist){
    ...
    re_list_t *dst[NOPD], *src[NOPD];
    int nuse, ndef;
    valset_u vt_lo = {0}, vt_hi = {0};

    traverse_inst_operand(inst, src, dst, re_uselist, re_deflist, &nuse, &ndef);
    LOG(stdout, "umull_resolver: nuse = %d, ndef = %d\n", nuse, ndef);
    assert(nuse == 2 && ndef == 2);

    if (CAST2_USE(src[0]->node)->val_known && CAST2_USE(src[1]->node)->val_known) {
        uint64_t res = (uint64_t)CAST2_USE(src[0]->node)->val.dword * (uint64_t)CAST2_USE(src[1]->node)->val.dword;
        vt_lo.dword = (uint32_t)(res & 0xFFFFFFFF);
        vt_hi.dword = (uint32_t)(res >> 32);
        if (CAST2_DEF(dst[0]->node)->val_stat & AfterKnown) {
            assert_val(dst[0], vt_lo, false);
            assert_val(dst[1], vt_hi, false);
        }
        if (!(CAST2_DEF(dst[1]->node)->val_stat & AfterKnown)) {
            assign_def_after_value(dst[0], vt_lo);
            add_to_deflist(dst[0], re_deflist);
            assign_def_after_value(dst[1], vt_hi);
